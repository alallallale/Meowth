...
function idConfirm(id) {
	...
	if (id.length <= 20 && id.length >= 6) {
		if (idRegExp.test(id)) {
			$
					.ajax({
						...
						success : function(data) {
							if (data == "true") {
								$('#idOverlapTest').text("※ 사용가능한 아이디 입니다.");
								$('#idOverlapTest').css('color', 'green');
							} else if (data == "false") {
								$('#idOverlapTest').text("※ 이미 사용중인 아이디 입니다.");
								$('#idOverlapTest').css('color', 'red');
							} else {
								$('#idOverlapTest').text("아이디를 입력해 주세요");
								$('#idOverlapTest').css('color', 'black');
							}
						},
						error : function() {
							alert("idConfirm err");
			...
			$('#idOverlapTest').text("※ 영어와 숫자로 6~20글자로만 가능합니다.");
			$('#idOverlapTest').css('color', 'red');
		}
	} else {
		$('#idOverlapTest').text("※ 영어와 숫자로 6~20글자로만 가능합니다.");
...
function nameConfirm(name) {
	...
	if (name.length <= 20 && name.length >= 2) {
					...
					.ajax({
						...
						success : function(data) {
							if (data == "true") {
								...
								$('#nameOverlapTest').text("※ 이미 사용중인 이름 입니다.");
								$('#nameOverlapTest').css('color', 'red');
							} else {
		...
		$('#nameOverlapTest').text("※ 이름은 2 글자 이상 20글자 이하이고 특수문자는 사용할 수 없스니다.");
		$('#nameOverlapTest').css('color', 'red');
	}
}
...
function overlapConfirm() {
	if ($('#idOverlapTest').text() == "※ 사용가능한 아이디 입니다."
			&& $('#passwordOverlapTest').text() == "※ 사용가능한 비밀번호 입니다."
			&& $('#nameOverlapTest').text() == "※ 사용가능한 이름 입니다."
			&& $('#phoneOverlapTest').text() == "※ 전화번호가 맞습니다."
			&& $('#emailOverlapTest').text() == "※ 이메일 입니다."
