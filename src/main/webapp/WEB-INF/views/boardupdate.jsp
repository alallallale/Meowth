						...
						<form action="boardupdate" method='post'>
							<div class="form-label-group">
								...
								<input type="text" name="p_writer"
									...
									class="form-control" required readonly> <label
									for="p_writer">작성자</label>
								<%
									} else {
								%>
								<script>
							...
							<div class="form-label-group">
								<input type="password" name="p_password" id="p_password"
									...
									value="<%=boardvo.getP_password()%>"> <label
									for="p_password">비밀번호</label>
							</div>
							<div class="form-label-group">
								<input type="text" name="p_title" id="p_title"
									class="form-control" placeholder="제목" required="required"
									value="<%=boardvo.getP_title()%>"> <label
									for="p_title">제목</label>
							</div>
							<div>
								<textarea class="form-control" name="p_content" id="summernote"
									rows="5" placeholder="내용" required="required"><%=boardvo.getP_content()%></textarea>
							</div>
							<input type="hidden" name="p_seq" value="<%= boardvo.getP_seq() %>">
